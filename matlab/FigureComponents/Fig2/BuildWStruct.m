function whisk_struct = BuildWStruct(data_directory)
...
for b = 1:numel(hilbert_feature)
    ...
    if exist([data_directory 'Whisking\' fileName '.mat'], 'file')
        ...
        whisk_struct.(hilbert_feature{b}) = load([data_directory 'Whisking\' fileName '.mat']);
    else
        disp('No whisking structure found. Building from scratch')
        if ~exist('U')
            disp('Loading complete all neural recordings')
            U = load([data_directory 'Raw\excitatory_all.mat']); %L5b excitatory cells recorded by Jon and Phil
        end
    disp(['Building whisking structure for ' hilbert_feature{b}])
    disp('This may take some time so consider using pre-built structures')
