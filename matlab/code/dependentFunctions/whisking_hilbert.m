function whisking_hilbert(array,displayOpt)
...
for i = 1:length(whiskingCells)
    currArray = array{whiskingCells(i)};
    ...
    SEM_response = cell2mat(cellfun(@(x) cellfun(@(y) nanstd(y)./sqrt(numel(y)),x), response,'uniformoutput',0));
    ...
    %tmp = interp1(average_stimulus(:,1),average_response(:,1),linspace(min(average_stimulus(:,1)),max(average_stimulus(:,1)),numInterpPts));
    ...
    if willdisplay
        ...
        for j = 1:size(average_response,2)
            ...
            if ~isempty(pcompare{j})
                ...
                hold on; scatter(average_stimulus(peakResponse,j),average_response(peakResponse,j)*1000,'filled','g')
                scatter(average_stimulus(firstDiff,j),average_response(firstDiff,j)*1000,'filled','r')
            end
        end
    end
